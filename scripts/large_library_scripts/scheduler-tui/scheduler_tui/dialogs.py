...
class AddScreen(ModalScreen[Optional[dict]]):
    ...
    def __init__(self, libraries: List[str], default_library: str = "") -> None:
        super().__init__()
        self.libraries = libraries
        self.default_library = default_library

    def compose(self) -> ComposeResult:
        library_options = [("<driver default>", "")] + [(l, l) for l in self.libraries]
        ...
        with Vertical(id="add-box"):
            ...
            yield Label("Model id  (its SIF must be at /shared/sif-files/<id>.sif)",
                        classes="field-label")
            yield Input(placeholder="eos4k4f_v1", id="in-model")

            yield Label("Run mode", classes="field-label")
            yield Select([(m, m) for m in MODES], value="ersilia",
                         allow_blank=False, id="in-mode")

            yield Label("Library", classes="field-label")
            yield Select(library_options, value=initial, allow_blank=False, id="in-library")

            yield Label("Wave size  (blank = driver default; 1..1000)", classes="field-label")
            yield Input(placeholder="1000", id="in-wave")

            yield Label("SLURM partition  (blank = driver default)", classes="field-label")
