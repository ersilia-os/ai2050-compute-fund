...
class Runner:
    ...
    #: populated with the last transport error, for the UI's stale banner
    ...
    def run(self, *args: str, timeout: Optional[int] = None) -> Tuple[int, str, str]:
        ...
        try:
            ...
            raise RunnerError(
                f"timed out after {self.timeout}s running: {' '.join(argv)}"
            ) from exc
...
def build_runner(
    ...
    locally. When running locally we can fall back to the ctl script sitting next
    to this package's repo checkout, which makes ``pip install -e .`` development
...
def _find_local_ctl() -> Optional[str]:
    """Locate sched-ctl.sh for a local run: deployed copy, then repo sibling."""
    candidates = [
        DEFAULT_REMOTE_CTL,
        # ../../scheduler/sched-ctl.sh relative to this file in the repo layout
        os.path.normpath(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "..", "..", "scheduler", "sched-ctl.sh",
            )
        ),
    ]
