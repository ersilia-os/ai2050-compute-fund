...
def _parse_status_tsv(lines: List[str]) -> Dict[str, Dict[str, str]]:
    ...
    for line in lines:
        ...
        key = parts[0]
        values = parts[1:]
        out[key] = {name: (values[i] if i < len(values) else "") for i, name in enumerate(cols)}
    return out
...
def _parse_state_tsv(lines: List[str]) -> Dict[str, Dict[str, str]]:
    ...
    cols = ["idx", "model", "mode", "library", "status", "done", "total",
    ...
    for line in lines:
        ...
        if len(parts) < 5:
            continue
        row = {name: (parts[i] if i < len(parts) else "") for i, name in enumerate(cols)}
        out[row["model"]] = row
    return out
...
def parse_dump(text: str) -> Snapshot:
    ...
    snap.libraries = [l.strip() for l in sections.get("libraries", []) if l.strip()]
