#!/bin/bash
# =============================================================================
# Wave orchestrator for billion-scale libraries (e.g. Enamine_Real_Sample_1.41B)
# =============================================================================
...
#
# Run it on the HEAD NODE inside tmux/nohup (a full 1.41B run can take days):
#   tmux new -s waves
#   S3_BUCKET=ai2050-ersilia-cluster \
#     /shared/scripts/submit-ersilia-waves.sh eos4k4f_v1 Enamine_Real_Sample_1.41B 1000 cpu-queue
#
# Usage: submit-ersilia-waves.sh <model_id> <library_name> [wave_size=1000] [queue=cpu-queue]
# =============================================================================
...
if [ -z "$MODEL_ID" ] || [ -z "$LIBRARY_NAME" ]; then
    ...
    echo "Example: $0 eos4k4f_v1 Enamine_Real_Sample_1.41B 1000 cpu-queue"
    exit 1
fi
...
if [ ! -f "$RUN_JOB" ]; then
    echo "ERROR: worker script not found: $RUN_JOB (deploy run-ersilia-wave-job.sh to /shared/scripts)"
    exit 1
fi
...
echo "=========================================="
echo "Ersilia Wave Orchestrator"
echo "=========================================="
echo "Model      : $MODEL_ID"
...
verify_wave() {
    ...
    while read -r inpath; do
        ...
        out="${OUTPUT_DIR}/${MODEL_ID}_results_${num}.csv"
        if [ ! -f "$out" ]; then
...
sizing_warning() {
    ...
    sample=$(ls "$OUTPUT_DIR"/${MODEL_ID}_results_*.csv 2>/dev/null | head -1) || return 0
    [ -n "$sample" ] || return 0
...
while [ "$START" -le "$REMAINING_COUNT" ]; do
    ...
    aws s3 sync "$OUTPUT_DIR/" "$S3_OUTPUT" \
        --exclude "*" --include "${MODEL_ID}_results_*.csv" >/dev/null
    while read -r inpath; do
        num=$(basename "$inpath" .csv | grep -oP '\d+$')
