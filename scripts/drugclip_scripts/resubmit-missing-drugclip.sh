...
#
# Usage:
#   bash /shared/scripts/drugclip_scripts/resubmit-missing-drugclip.sh <library_name> [queue]
#
# Examples:
#   bash /shared/scripts/drugclip_scripts/resubmit-missing-drugclip.sh Enamine_Hit_Locator_460K
#   bash /shared/scripts/drugclip_scripts/resubmit-missing-drugclip.sh Molport_Screening_Compounds_5.3M gpu-queue
...
for INPUT_FILE in $(ls "${INPUT_DIR}"/*_chunk_*.csv 2>/dev/null | sort); do
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/${LIBRARY_NAME}_drugclip_${CHUNK_NUM}.csv"
    BISECT_DIR="${OUTPUT_DIR}/bisect_${CHUNK_NUM}"

    if [ ! -f "$OUTPUT_FILE" ]; then
        # Check if a bisect is complete and just needs merging
        if [ -d "$BISECT_DIR" ] && [ -n "$(ls "${BISECT_DIR}"/result_*.csv 2>/dev/null)" ]; then
            echo "  BISECT   : chunk_${CHUNK_NUM} — merging results"
            bash "${SCRIPT_DIR}/merge-drugclip-bisect.sh" "$LIBRARY_NAME" "$CHUNK_NUM"
    ...
    # Check row count vs input row count
    IN_ROWS=$(( $(wc -l < "$INPUT_FILE") - 1 ))
    OUT_ROWS=$(( $(wc -l < "$OUTPUT_FILE") - 1 ))

    if [ "$IN_ROWS" -ne "$OUT_ROWS" ] 2>/dev/null; then
