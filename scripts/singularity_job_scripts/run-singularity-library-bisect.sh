...
# Run one sub-chunk of a divide-and-conquer search on a failed singularity library chunk.
#
# Submitted by bisect-singularity-library-chunk.sh (initial split) or recursively by
# this script itself (subsequent splits). Never call this script directly.
#
# Args: MODEL_ID LIBRARY_NAME CHUNK_NUM BISECT_DIR TASKS_FILE [QUEUE]
#
...
echo "=========================================="
echo "Singularity Library Bisect Job"
echo "=========================================="
...
candidates = (
    ...
    glob.glob(f"/fsx/output/{library}/{model_id}/{model_id}_*.csv")
)
...
PYEOF
    ...
    JOB_OUTPUT=$(sbatch \
        ...
        /shared/scripts/singularity/run-singularity-library-bisect.sh \
        "$MODEL_ID" \
        "$LIBRARY_NAME" \
        "$CHUNK_NUM" \
        "$BISECT_DIR" \
