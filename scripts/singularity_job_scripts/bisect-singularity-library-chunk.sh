#!/bin/bash
# Divide-and-conquer resubmission for a failed singularity library chunk.
#
# Usage:
#   bisect-singularity-library-chunk.sh <model_id> <library_name> <chunk_num> [queue]
#
# Example:
#   bisect-singularity-library-chunk.sh mtb-public-models Coconut_715K 042
...
echo "=========================================="
echo "Bisect Singularity Library Chunk"
echo "=========================================="
echo "Model:    $MODEL_ID"
...
JOB_OUTPUT=$(sbatch \
    ...
    "${SCRIPT_DIR}/run-singularity-library-bisect.sh" \
    "$MODEL_ID" \
    "$LIBRARY_NAME" \
    "$CHUNK_NUM" \
    "$BISECT_DIR" \
...
echo "Merge:    bash ${SCRIPT_DIR}/merge-singularity-library-bisect.sh $MODEL_ID $LIBRARY_NAME $CHUNK_NUM"
